...
- **訊息格式是「標題 + 網址」的純文字,不用自訂 embed**:送出後由 Discord 自己展開 YouTube 預覽(有播放器的那種)。
  之所以不做自訂 embed —— 訊息一旦帶 embed,Discord 就**不再自動展開連結**,預覽會退化成靜態圖;
  而且 YouTube 的影片說明動輒上千字(曲目表、製作資訊、著作權聲明),塞進 embed 會洗整個頻道。
  ⚠ bot 需要該頻道的「嵌入連結 (Embed Links)」權限,否則網址只會是純文字、不會有預覽。
